...
_price_cache      = {}
_price_cache_time = 0
PRICE_CACHE_TTL   = 900  # 15 min cache — CoinGecko context only, Binance handles live prices
...
def api_state():
    ...
    # Always seed ALL coins from COIN_SYMBOLS so dashboard is never empty
    for coin_id, symbol in COIN_SYMBOLS.items():
        if coin_id not in prices_data:
            prices_data[coin_id] = {
                "price": 0, "symbol": symbol, "name": symbol,
