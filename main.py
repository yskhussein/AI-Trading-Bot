...
COINS = [
    ...
    "ripple","cardano","avalanche-2","chainlink","polkadot",
    "tron","sui","pepe","floki","bonk",
    "the-open-network","near","aptos","injective-protocol","sei-network",
]
COIN_SYMBOLS = {
    "bitcoin":"BTC","ethereum":"ETH","solana":"SOL","dogecoin":"DOGE","shiba-inu":"SHIB",
    "ripple":"XRP","cardano":"ADA","avalanche-2":"AVAX","chainlink":"LINK","polkadot":"DOT",
    "tron":"TRX","sui":"SUI","pepe":"PEPE","floki":"FLOKI","bonk":"BONK",
    "the-open-network":"TON","near":"NEAR","aptos":"APT","injective-protocol":"INJ","sei-network":"SEI",
}
BINANCE_SYMBOLS = {
    "bitcoin":"BTCUSDT","ethereum":"ETHUSDT","solana":"SOLUSDT","dogecoin":"DOGEUSDT","shiba-inu":"SHIBUSDT",
    "ripple":"XRPUSDT","cardano":"ADAUSDT","avalanche-2":"AVAXUSDT","chainlink":"LINKUSDT","polkadot":"DOTUSDT",
    "tron":"TRXUSDT","sui":"SUIUSDT","pepe":"PEPEUSDT","floki":"FLOKIUSDT","bonk":"BONKUSDT",
    "the-open-network":"TONUSDT","near":"NEARUSDT","aptos":"APTUSDT","injective-protocol":"INJUSDT","sei-network":"SEIUSDT",
}
RISK_PER_TRADE  = 0.10
...
- On re-entry after a loss: analyse if conditions changed, if still bullish BUY immediately
    ...
    for coin_id in COINS:
        try:
            ...
            can_buy = (
                ...
                coin_trades < MAX_TRADES_PER_COIN and   # max 3 per coin
                len(state["open_trades"]) < MAX_OPEN_TRADES and
                state["cash_balance"] >= price * 0.001  # only need tiny cash min
            )
            if can_buy:
...
async def cmd_topup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Add funds to paper balance. Usage: /topup 10000"""
    try:
        ...
        portfolio_val = get_portfolio_value(prices_data)
        await update.message.reply_text(
            f"💰 <b>TOPPED UP ${amount:,.2f}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"💵 Cash balance: ${state['cash_balance']:,.2f}\n"
            f"📊 Portfolio value: ${portfolio_val:,.2f}\n\n"
            f"Quick options:\n"
            f"/topup 10000 — add $10K\n"
            f"/topup 100000 — add $100K\n"
            f"/topup 1000000 — add $1M\n"
            f"/topup 5000000 — add $5M (max)",
            parse_mode=ParseMode.HTML,
        )
        ...
        await update.message.reply_text(
            ...
            "/topup 500000\n"
            "/topup 5000000\n\n"
            f"Current cash: ${state['cash_balance']:,.2f}"
        )
...
async def scheduler(app_tg: Application):
    ...
    await notify(
        ...
        f"⚡ Scans: Every 60s | 20 coins | NO position limit\n"
        f"💰 Risk: AI-decided | Compounding ON | Re-entry ON | Doubling ON\n"
  ...
  <div class="grid grid-2">
    ...
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
        <button class="btn success" onclick="topup(10000)">+ $10,000</button>
        <button class="btn success" onclick="topup(50000)">+ $50,000</button>
        <button class="btn success" onclick="topup(100000)">+ $100,000</button>
        <button class="btn success" onclick="topup(500000)">+ $500,000</button>
        <button class="btn success" onclick="topup(1000000)">+ $1,000,000</button>
        <button class="btn success" onclick="topup(5000000)">+ $5,000,000</button>
      </div>
      <div style="display:flex;gap:8px">
        <input type="number" id="customTopup" placeholder="Custom amount (max $5,000,000)"
          min="1" max="5000000"
          style="background:var(--bg0);border:1px solid var(--border);color:var(--text);
          font-family:var(--mono);font-size:0.82rem;padding:8px 12px;border-radius:3px;flex:1"/>
        <button class="btn success" onclick="topup(parseFloat(document.getElementById('customTopup').value))">+ Add</button>
      </div>
    </div>
    <div class="card">
...
async function topup(amount){
  ...
  try{
    const r=await fetch('/api/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'topup',amount})});
    const d=await r.json(); showToast(d.message,'success'); fetchAll();
  }catch(e){showToast('Error','error')}
}
...
def api_control():
    ...
    if action == "pause":
        ...
        amount = max(1, min(amount, 5_000_000))   # cap at $5M
        state["cash_balance"] += amount
        save_state()
        return jsonify({"message": f"💰 Topped up ${amount:,.2f} — new cash: ${state['cash_balance']:,.2f}"})
