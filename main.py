...
_price_cache      = {}
_price_cache_time = 0
PRICE_CACHE_TTL   = 60   # 60s cache — matches scan interval
...
def get_binance_prices() -> dict:
    ...
    try:
        ...
        # Batch fetch all symbols in one call
        r = requests.get(
...
- Be aggressive when setup is clear, patient when it is not"""
    ...
    # Get Binance real-time data for scalping
    ...
    user_msg = f"""⚡ SCALP/DAY TRADE ANALYSIS — {symbol}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
BINANCE LIVE PRICE: ${b.get('price', price_data.get('price','N/A'))}
Bid/Ask:       ${b.get('bid',0):,.6f} / ${b.get('ask',0):,.6f} | Spread: {b.get('spread_pct',0):.4f}% {'✅ TIGHT' if b.get('spread_pct',1) < 0.05 else '⚠️ WIDE'}
Micro Momentum:{b.get('micro_mom',0):+.4f}% (price vs weighted avg) {'🔥 BULLISH' if b.get('micro_mom',0) > 0.02 else '❄️ BEARISH' if b.get('micro_mom',0) < -0.02 else '➡️ FLAT'}
Trade Count:   {b.get('trades_count',0):,} trades in 24h {'⚡ HIGH ACTIVITY' if b.get('trades_count',0) > 100000 else ''}
Range Position:{scalp_signal.get('range_pos',0.5):.2f} (0=at 24h low, 1=at 24h high)
Scalp Signal:  {'✅ READY — ' + scalp_signal.get('direction','').upper() + ' (strength: ' + str(scalp_signal.get('strength',0)) + ')' if scalp_signal.get('scalp_ready') else '❌ NOT READY'}
CoinGecko 24h: {price_data.get('change_24h',0):+.2f}% | 7d: {price_data.get('change_7d',0):+.2f}%
Volume 24h:    ${price_data.get('volume_24h',0):,.0f} {'⚡ VOLUME SPIKE' if momentum.get('volume_spike') else ''}
Momentum:      {momentum.get('trend','?').upper()} ({momentum.get('momentum_pct',0):+.3f}% / period)

TECHNICAL INDICATORS:
RSI(14):       {technicals['rsi']} — {'🟢 OVERSOLD' if technicals['rsi'] < 30 else '🔴 OVERBOUGHT' if technicals['rsi'] > 70 else '⚪ NEUTRAL'}
MACD:          {technicals['macd']} — Trend: {technicals['macd_trend'].upper()}
Bollinger:     {technicals['bb_position'].upper()} (B%: {technicals['bb_pct_b']:.2f})
TA Consensus:  {technicals['signal'].upper()}

OPEN POSITION: {open_trade_context}
CASH AVAILABLE: ${state['cash_balance']:,.2f}

LATEST NEWS & SENTIMENT:
{news}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Pick the best strategy (scalp/momentum/news/reversal) and decide.
JSON only:"""
