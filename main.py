...
SCAN_INTERVAL  = 900
DAILY_LOSS_CAP  = 0.20
STOP_LOSS_PCT   = 0.05   # auto-sell if trade drops 5%
TAKE_PROFIT_PCT = 0.15   # auto-sell if trade gains 15%
STATE_FILE      = "/app/state.json"   # persistent storage path
PAPER_BALANCE  = 10000.0

# ── PRICE CACHE (avoids CoinGecko 429 rate limit) ─────────────────────────────
_price_cache = {}
_price_cache_time = 0
...
RSI (14): {technicals['rsi']} {'[OVERSOLD - potential BUY]' if technicals['rsi'] < 30 else '[OVERBOUGHT - potential SELL]' if technicals['rsi'] > 70 else '[NEUTRAL]'}
MACD: {technicals['macd']} | Trend: {technicals['macd_trend'].upper()}
Bollinger Bands: {technicals['bb_position'].upper()} (position: {technicals['bb_pct_b']:.2f})
Combined TA Signal: {technicals['signal'].upper()}
...
def paper_buy(coin_id: str, price: float) -> dict:
    ...
    logger.info(f"PAPER BUY {coin_id}: qty={qty:.6f} @ ${price} | SL=${stop_price} TP=${target_price}")
    save_state()
    return trade
        ...
        try:
            ...
            if action == "BUY" and confidence >= MIN_CONFIDENCE and not open_trade:
                if state["cash_balance"] >= price * 0.01:
                    ...
                    await send_trade_approval(coin_id, decision, price_data)
...
function renderActivityLog(d){
  ...
  document.getElementById('activityLog').innerHTML=log.map(e=>{
    ...
    return `<div class="activity-line">
      ...
      <span class="activity-text">${e.confidence}% conf${rsi}${ta} — ${(e.key_news||'').slice(0,60)}${e.key_news&&e.key_news.length>60?'…':''}</span>
    </div>`;
