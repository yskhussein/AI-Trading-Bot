...
def futures_open(coin_id: str, price: float, direction: str, decision: dict) -> dict | None:
    ...
    margin   = state["futures_balance"] * FUTURES_RISK_PCT
    margin   = max(1.0, min(margin, state["futures_balance"] * 0.20))  # cap at 20%

    if margin > state["futures_balance"]:
        logger.warning(f"Insufficient futures balance for {coin_id}")
        return None
